<div class="suggested-articles p142 mw14 center center16">
    <div class="suggested-main">
        @if($articles->isEmpty())
            <p class="wt font1 ct no-articles">{{ __('messages.no_article') }}</p>
            <style>
                .suggested-main {
                ...
                <div class="article-container wt">
                    <div class="article">
                        ...
                        <div class="overlay-container">
                            <a href="{{ route('articles.category', strtolower($article->category)) }}" class="cat-label ds wt font1 {{ strtolower($article->category) }}">{{ ucfirst(__('messages.' . $article->category)) }}</a>
                            <a class="text" href="{{ route('articles.show', $article->id) }}">
                                <p class="wt font1 title ds2">{{ $article->title }}</p>
                                <p class="gt font1 date-author ds transparent">{{ $article->created_at ? $article->created_at->format('d.m.Y') : __('messages.no_date') }}  •  {{ $article->author }}</p>
                            </a>
                        </div>
